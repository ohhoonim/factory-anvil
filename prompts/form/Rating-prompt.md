...
2. 표준 파일 구성 (7종):
   ...
   - Rating.test.ts (Vitest 및 Playwright 테스트)
   - index.ts (통합 export)
3. 네임스페이스 및 명명 규칙:
...
[작성 조건 - Rating.wc.ts]
1. `LitElement`를 상속받아 클래스를 구현하고, `@customElement('biz-rating')` 디코레이터를 사용하여 커스텀 엘리먼트로 등록하세요.
2. 2단계에서 생성한 `RatingTemplate` 및 `Rating.css.ts`의 `ratingStyles`를 임포트하세요.
...
````text
[요청 사항]
1~4단계에서 작성된 코드와 요구사항 정의서를 바탕으로 컴포넌트 품질 관리를 위한 Storybook 문서 파일(`Rating.stories.ts`)과 단위/통합 테스트 파일(`Rating.test.ts`) 코드를 작성해 주세요.
...
[출력 형식]
- 각 파일별 경로(`src/components/Rating/Rating.stories.ts`, `src/components/Rating/Rating.test.ts`)를 명시하고 해당 코드 블록만 출력하세요.
