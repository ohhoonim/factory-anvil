# DateTimePicker Biz-UI 웹 컴포넌트 개발 프롬프트 체인
...
[Biz-UI 개발 컨벤션]
1. 디렉터리 구조: 모든 파일은 `src/components/DateTimePicker/` 아래에 위치합니다.
2. 표준 파일 구성 (7종):
   - DateTimePicker.ts (코어 Lit 템플릿)
   - DateTimePicker.css.ts (Lit css 태그드 템플릿 기반 전용 스타일)
...
````text
[요청 사항]
1단계에서 전달받은 요구사항 정의서를 바탕으로 Biz-UI 컴포넌트의 코어 템플릿(`DateTimePicker.ts`)과 전용 Lit 스타일시트(`DateTimePicker.css.ts`) 코드를 작성해 주세요.

[작성 조건 - DateTimePicker.ts]
...
2. 스타일은 `export const dateTimePickerStyles = css`...`` 형태의 Lit css 태그드 템플릿 모듈로 생성하세요.
3. `:host` 블록 내에 `--biz-date-time-picker-*` 형태의 CSS Custom Properties(디자인 토큰)를 기본값과 함께 정의하세요.
4. 루트 클래스명은 `biz-date-time-picker`으로 지정하세요.
...
[출력 형식]
- 파일별 경로(`src/components/DateTimePicker/DateTimePicker.ts`, `src/components/DateTimePicker/DateTimePicker.css.ts`)를 명시하고 해당 코드 블록만 출력하세요.
...
[출력 형식]
- 파일 경로(`src/components/DateTimePicker/DateTimePicker.wc.ts`)를 명시하고 해당 코드 블록만 출력하세요.
...
````text
[요청 사항]
1~3단계에서 작성된 요구사항 정의서 및 웹 컴포넌트 코드를 바탕으로 React 래퍼 파일(`DateTimePicker.react.ts`), 컴포넌트 통합 모듈(`index.ts`), 그리고 패키지 통합 진입점(`src/react.ts`) 구문을 작성해 주세요.

[작성 조건 - DateTimePicker.react.ts]
1. `@lit/react` 패키지의 `createComponent` 함수를 사용하여 React 컴포넌트를 정의하세요.
2. 3단계에서 생성한 `DateTimePickerWc` 클래스와 커스텀 엘리먼트 태그명(`biz-date-time-picker`)을 연결하세요.
...
[작성 조건 - index.ts]
1. `src/components/DateTimePicker/index.ts` 경로에 작성하세요.
2. 코어 템플릿(`DateTimePickerTemplate`), 스타일(`dateTimePickerStyles`), 웹 컴포넌트 클래스(`DateTimePickerWc`), React 래퍼 컴포넌트(`DateTimePicker`), 그리고 주요 TypeScript Interface/Type들을 모두 export 하세요.

[작성 조건 - src/react.ts]
1. 패키지 루트의 `src/react.ts` 진입점에 신규 생성된 React 래퍼 컴포넌트를 re-export 하는 구문을 작성해 주세요. (예: `export * from './components/DateTimePicker/DateTimePicker.react';`)
