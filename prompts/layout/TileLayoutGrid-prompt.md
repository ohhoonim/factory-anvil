...
3. 네임스페이스 및 명명 규칙:
   ...
   - 템플릿 함수 파라미터 'host'의 인터페이스 export 명칭:`TileLayoutGridHost` 
...
6. host타입은 `TileLayoutGridHost` 명칭으로 export 하세요.
...
1. `LitElement`를 상속받고, 2단계에서 생성한 `TileLayoutGridHost`를 implements 하여  클래스를 구현하고, `@customElement('biz-tile-layout-grid')` 디코레이터를 사용하여 커스텀 엘리먼트로 등록하세요.
2. 2단계에서 생성한 `TileLayoutGridTemplate` 및 `TileLayoutGrid.css.ts`의 `tileLayoutGridStyles`를 임포트하세요. `TileLayoutGridHost`를 type 임포트하세요.
...
2. 컴포넌트의 Host 속성 타입(e.g., `TileLayoutGridHost`)에 `Required<T>`를 적용하여 모든 프로퍼티를 필수화한 후, Slot 관련 컨트롤 키를 추가한 `Args` 타입을 정의하세요.
...
[출력 형식]
- 파일 경로(`src/components/TileLayoutGrid/TileLayoutGrid.stories.ts`)를 명시하고 해당 코드 블록만 출력하세요.
